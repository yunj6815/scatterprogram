...
def display_smart_report(df, x_col, y_col):
    ...
    # --- [Case 2] 문자(범주) vs 숫자 상세 보고서 ---
        ...
        if total_outliers > 0:
            ...
            st.write("- **해석:** 동일한 조건 내에서도 편차가 큰 샘플이 존재함을 의미합니다.")
        else:
            st.write("- **이상치(Outlier):** 각 그룹 내에서 통계적으로 크게 벗어난 극단적인 데이터는 발견되지 않았습니다.")

        st.markdown("### 4. 결론 및 요약")
        if mean_diff > (df_clean[y_col].max() - df_clean[y_col].min()) * 0.1:
